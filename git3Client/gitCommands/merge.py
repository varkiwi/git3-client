...
def merge(source_branch):
    ...
    if source_branch is None:
        ...
        findings = re.findall(
            "^([ABCDEFabcdef0-9]+)\s+branch (\w|')+", fetch_head_content
        )
        if len(findings) == 0:
            remote_sha1 = None
        else:
        ...
        source_branch_head_path = os.path.join(
            repo_root_path, ".git/refs/heads/", source_branch
        )
        if not os.path.exists(source_branch_head_path):
            ...
            findings = re.findall(
                "([ABCDEFabcdef0-9]*) refs\/remotes\/origin\/{}".format(source_branch),
                packed_refs_content,
            )
            if len(findings) == 0:
                remote_sha1 = None
    ...
    get_subtree_entries(
        ancestor_commit.splitlines()[0][5:45].decode(), "", ancestor_entries
    )
                ...
                if os.path.exists(path):
                    ...
                    write_file(
                        ...
                        "Merge branch '{}' of {} into {}\n\n# Conflicts\n# \t{}\n".format(
                            source_branch, repository[2], activeBranch, f
                        ).encode(),
                    )
    ...
    if not had_conflict:
        commit(
            "Merging {} into {}".format(source_branch, activeBranch),
...
def three_way_merge(x, a, b, conflict_commit_one, conflict_commit_two):
    ...
    while (index_a < len(xa)) and (index_b < len(xb)):
        ...
        if (xa[index_a][2:] == xb[index_b][2:]) and (
            xa[index_a].startswith("- ") or xb[index_b].startswith("- ")
        ):
            index_a += 1
            index_b += 1
            continue
