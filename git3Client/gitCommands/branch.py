...
def create_branch(command, newBranchName):
    ...
    # If not,
    currentHeadRef = (
        read_file("{}/.git/HEAD".format(repo_root_path))
        .decode("utf-8")
        .split("ref:")[1]
        .strip()
    )
    ...
    if os.path.isfile("{}/.git/{}".format(repo_root_path, currentHeadRef)):
        ...
        commitHash = read_file(
            "{}/.git/{}".format(repo_root_path, currentHeadRef)
        ).decode("utf-8")
        write_file(
            "{}/.git/refs/heads/{}".format(repo_root_path, newBranchName),
            commitHash,
            binary="",
        )
    else:
        ...
        if command == "branch":
            print(
                "fatal: Not a valid object name: '{}'.".format(
