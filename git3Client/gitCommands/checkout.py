...
def checkout(branch):
    ...
    if os.path.isfile("{}/.git/refs/heads/{}".format(repo_root_path, branch)):
        ...
        target_commit_hash = (
            read_file("{}/.git/refs/heads/{}".format(repo_root_path, branch))
            .decode("utf-8")
            .strip()
        )
    else:
        ...
        if os.path.isfile("{}/.git/FETCH_HEAD".format(repo_root_path)):
            fetch_head_content = read_file(
                "{}/.git/FETCH_HEAD".format(repo_root_path)
            ).decode("utf-8")
            target_commit_hash = (
                fetch_head_content.split("branch '{}'".format(branch))[0]
                .split("\n")[-1]
                .split("\t")[0]
                .strip()
            )
        # if it does not exist, we check if packed-refs exists
            ...
            packed_refs_content = read_file(
                "{}/.git/packed-refs".format(repo_root_path)
            ).decode("utf-8")
            ...
            print(
                "error: pathspec '{}' did not match any file(s) known to git3".format(
                    branch
                )
            )
            exit(1)
    ...
    current_commit_hash = (
        read_file("{}/.git/refs/heads/{}".format(repo_root_path, active_branch))
        .decode("utf-8")
        .strip()
    )
    ...
    if from_packed_refs:
        print(
            "Branch '{}' set up to track remote branch '{}' from 'origin'.".format(
                branch, branch
            )
        )
        write_file(
            "{}/.git/refs/heads/{}".format(repo_root_path, branch),
            target_commit_hash,
            binary="",
        )
    ...
    if current_commit_hash == target_commit_hash:
        ...
        write_file(
            "{}/.git/HEAD".format(repo_root_path, branch),
    ...
    for filename in commit_entries:
        ...
        write_file(
            "{}/{}".format(repo_root_path, filename), data.decode("utf-8"), binary=""
        )
