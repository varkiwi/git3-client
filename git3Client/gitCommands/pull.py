...
from git3Client.gitInternals.gitCommit import (
    get_all_local_commits,
    unpack_files_of_commit,
    read_commit_entries,
)
...
def pull():
    ...
    if len(remote_to_local_difference) == 0 and len(local_to_remote_difference) > 0:
        ...
        remote_commits = list(
            filter(lambda x: x["sha1"] in remote_to_local_difference, remote_commits)
        )
        ...
        for filename in commit_entries:
            ...
            write_file(
                "{}/{}".format(root_path, filename), data.decode("utf-8"), binary=""
            )
