...
def check_if_remote_ahead(remote_sha1):
    ...
    path_to_check = os.path.join(
        root_path, ".git", "objects", remote_sha1[:2], remote_sha1[2:]
    )
...
def push_tree(tree_hash: str, folder_name: str, remote_database: dict) -> str:
    ...
    for entry in entries:
        ...
        if (
            entry[1] not in subdirFiles
            or "commit_time" not in subdirFiles[entry[1]]
            or (
                int(remote_database["committer"]["date_seconds"])
                > int(subdirFiles[entry[1]]["commit_time"])
                and subdirFiles[entry[1]]["sha1"] != entry[2]
            )
        ):
            ...
            subdirFiles[entry[1]]["commit_message"] = remote_database[
                "currentCommitMessage"
            ]
            subdirFiles[entry[1]]["commit_time"] = remote_database["committer"][
                "date_seconds"
            ]
        ...
        if entry[0] == GIT_NORMAL_FILE_MODE:
            ...
            blob_to_push = {"type": "blob", "content": blob.decode(), "sha1": entry[2]}
        ...
        tree_entries.append(
            {"mode": entry[0], "name": entry[1], "cid": cid, "sha1": entry[2]}
        )
...
def push_commit(commit_hash, remote_commit_hash, remote_commit_cid, remote_database):
    ...
    for parent in parents:
        parent_cid = push_commit(
            parent, remote_commit_hash, remote_commit_cid, remote_database
        )
...
def push_data_to_storage(data):
    ...
    while True:
        try:
            ...
            time_to_sleep += 2
            print(
                f"Due to too many requests, we will have to wait for {time_to_sleep} seconds"
            )
    return data["cid"]
