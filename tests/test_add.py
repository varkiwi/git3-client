...
@unittest.mock.patch('git3Client.gitCommands.add.repository')
def test_add_file_and_file_in_dir(mocked_repository, empty_objects_dir, delete_index_file):
    mocked_repository.get_repo_path.return_value = os.getcwd()
    assert os.listdir(OBJECTS_DIR_PATH) == []
...
@unittest.mock.patch('git3Client.gitCommands.add.repository')
def test_adding_non_existing_file(mocked_repository, empty_objects_dir):
    mocked_repository.get_repo_path.return_value = os.getcwd()
