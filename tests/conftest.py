...
def fund_user(w3):
    to = w3.geth.personal.import_raw_key(
        hex(PRIVATE_KEY.private_numbers().private_value), ""
    )
...
def deploy_contracts(eth_tester, fund_user):
    ...
    for facet in facets_contracts:
        ...
        for function in contract.all_functions():
            function_signatures.append(
                Web3.toHex(
                    Web3.sha3(text=str(function).split("Function")[1].replace(">", ""))[
                        0:4
                    ]
                )
            )
        ...
        contract_information.append(
            [tx_receipt.contractAddress, function_signatures, facet[1]]
        )
    ...
    with open(
        os.path.join(
            ...
            "artifacts",
            "contracts",
            "GitContractRegistry.sol",
            "GitContractRegistry.json",
        )
    ) as f:
        contract_data = json.loads(f.read())
    contract = w3.eth.contract(
    ...
    tx_hash = contract.constructor(contract_information).transact(
        {"from": deploy_address}
    )
    tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash, 180)
    ...
    git3_factory = w3.eth.contract(
        abi=contract_data["abi"], bytecode=contract_data["bytecode"][2:]
    )
    tx_hash = git3_factory.constructor(deploy_address).transact(
        {"from": deploy_address}
    )
    tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash, 180)
...
def patch_git_factory_for_create(mocker, deploy_contracts):
    mocker.patch(
        "git3Client.gitCommands.create.get_factory_contract",
        return_value=deploy_contracts,
    )
...
def patch_push_data_to_storage_for_push(mocker):
    mocker.patch(
        "git3Client.gitCommands.push.push_data_to_storage", return_value="AwesomeCID"
    )
...
def patch_check_if_repo_created_for_push(mocker):
    mocker.patch(
        "git3Client.gitCommands.push.check_if_repo_created", return_value="true"
    )
...
@pytest.fixture
def prepare_local_repo_till_commit(
    cleanup_repository,
    create_file,
    add_file_to_index,
    create_local_config_file_with_identity_file,
):
    """
...
def mock_successful_push(mocker, request):
    for key in request.param:
        mocker.patch(
            "git3Client.gitCommands.push." + key, return_value=request.param[key]
        )
